          ...
          {filtered.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map((lesson, idx) => {
                ...
                const isPro = lesson.required_tier !== 'free'
                const styles = difficultyStyle(lesson.difficulty)
                const stepsDone = lesson.progress?.completed_steps?.length ?? 0
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`h-1 w-full ${isCompleted ? 'bg-green-400' : styles.bar}`} />
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex items-start gap-3 mb-2">
                        <div className={`w-8 h-8 rounded flex items-center justify-center shrink-0 mt-0.5 ${
                          isCompleted ? 'bg-green-100' : isInProgress ? 'bg-slate-900' : 'bg-slate-100'
                        }`}>
                          <span className={`text-xs font-bold ${
                            isCompleted ? 'text-green-700' : isInProgress ? 'text-white' : 'text-slate-600'
                          }`}>
                            {idx + 1}
                          </span>
                        </div>
                        <div>
                          <h3 className={`text-base font-semibold leading-snug mb-1 ${isCompleted ? 'text-slate-500' : 'text-slate-900'}`}>
                            {lesson.title}
                          </h3>
